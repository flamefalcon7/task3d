...
sequenceDiagram
    ...
    Note right of SUI: PTB chain (built by U5): kiosk::purchase →<br/>royalty_rule::pay → transfer_policy::confirm_request<br/>(Royalty + Lock + PersonalKiosk rules satisfied)
...
- Frontend (U5) PTB chain: `purchase_with_kiosk → royalty_rule::pay(policy, request, royalty_coin) → transfer_policy::confirm_request(policy, request)`. confirm_request consumes the hot potato; if omitted, tx aborts.
...
- `buildPurchaseWithKioskPtb(args)`: emits FULL chain: `purchase_with_kiosk` → `royalty_rule::pay(policy, request, royalty_coin)` → `transfer_policy::confirm_request(policy, request)`. The `royalty_rule::pay` call is internal to the builder — frontend callers cannot accidentally omit it.
...
- `buildPurchaseWithKioskPtb` → tx contains THREE Move calls: `purchase_with_kiosk` → `royalty_rule::pay` → `confirm_request`
