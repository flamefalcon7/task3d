...
const DEATH_RGB = new Color4(0.16, 0.13, 0.12, 1); // dark smoke — reads on the light card
const REWARD_RGB = new Color4(0.86, 0.5, 0.04, 1); // deep amber-gold
...
function createBurst(scene: Scene, tex: DynamicTexture, color: Color4, up: number): Burst | null {
  ...
  ps.blendMode = ParticleSystem.BLENDMODE_STANDARD; // alpha, not additive — reads on light bg
...
export function InGamePanel(): JSX.Element {
  const onSceneReady = ({ scene, camera, meshes }: LiveWellSceneContext) => {
    ...
    // Game-editor GRID floor (the "building a game" vibe) + a separate
    // shadow-only ground so the contact shadow shows without muddying the grid.
