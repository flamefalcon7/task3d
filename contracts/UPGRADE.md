...
**v7 (D-040) — fresh republish adding L1 license-policy ENFORCEMENT (plan-009 U2):** adds a private const `EPolicyRestricted = 38` + an `assert!` at the top of the private `launch_collection_internal` enforcing L1 license policy (RESTRICTED/ALLOW_LIST base → creator-only fork; PERMISSIONLESS → any payer). Covers both `launch_collection` and `launch_collection_with_tokens` (both route through the internal).

**Why a fresh republish, not a compatible `sui client upgrade`** (the important upgrade-design lesson): the change is *mechanically* upgrade-compatible (new private const ✅, private-fn body change ✅, public signatures + struct layouts unchanged ✅), and a compatible upgrade was attempted first (published-at `0x134807cd…`, since abandoned). But for an **enforcement** change a compatible upgrade is the wrong tool: Sui does **not** retire the prior package version, so the pre-enforcement bytecode at the old package id stays permanently callable — a hand-crafted PTB targeting it bypasses the new assert entirely. The gate would be honor-system only. A **fresh republish has no prior version of itself**, so the policy is genuinely enforced for every object published under the new id. (Found by the plan-009 code review: security + adversarial + api-contract all independently flagged the stale-package bypass.)

Bonus: a fresh republish keeps a **single package id** — no published-at/original-id split, so moveCall targets and struct/event type identity use the same id again (the upgrade path had forced a split that was a standing foot-gun).

**General rule this establishes:** prefer a compatible upgrade for additive *capability* (new fns/consts that don't bind existing callers). Use a **fresh republish when the change must DENY behavior the old version allowed** (access control, validation tightening) — otherwise the old version stays an open bypass. Republish cost here was low: re-bootstrap `ensure_collection_policy` (royalty-only `TransferPolicy<NftToken>`) + update `testnet.json` / `networkConfig.ts` / `Published.toml`; Plan 010 had not started so the fresh TransferPolicy ids carry no migration cost.

**v7 shipped 2026-05-21 (testnet):** `package_id 0x3f53506b076bb9e43fbf8fc1333375530aeb97ad54e2ad81fdd36a9d595d0861` (single id — original-id == published-at, version 1), `upgrade_cap 0xcd587052…`, `publisher 0xee62b464…`, `TransferPolicy<NftToken> 0x3ffa22b3…` (+ cap `0x76cc6960…`). Rules VecSet verified = **royalty only** (1 rule: `0xe308bb3e…::royalty_rule::Rule`). Superseded v6 `0x57e20a13…` (+ its abandoned upgrade published-at `0x134807cd…`). publish digest `Cdubzmx8TCWehYvp3jRBhvk7n9g3cxJa6YgFmUUicon1`, bootstrap digest `B8MXhFp1SbQJPVJKfZgQyeGZ1Y4tCfZnMUAbT4YGJeGQ`. Move tests cover the abort cases (`launch_collection_restricted_non_creator_aborts`, `launch_collection_restricted_creator_ok`, `launch_collection_allow_list_non_creator_aborts`, `launch_collection_with_tokens_restricted_non_creator_aborts`); live cross-wallet abort confirmation folds into the four-actor demo (U15). See `contracts/networks/testnet.json`.
